...
import com.example.diogosoares.githubsample.api.GithubAPI;
...
import com.example.diogosoares.githubsample.serializers.GithubRepoDeserializer;
...
public class MainActivity extends AppCompatActivity implements CredentialsDialog.ICredentialsDialogListener {
        ...
        repositoriesSpinner.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                if (parent.getSelectedItem() instanceof GithubRepo) {
                    ...
                    githubAPI.getIssues(githubRepo.owner, githubRepo.name).enqueue(issuesCallBack);
                }
            }
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.loadRepos_button:
                githubAPI.getRepos().enqueue(repoCallback);
                break;
            case R.id.send_comment_button:
                ...
                if (!newComment.isEmpty()) {
                    ...
                    githubAPI.postComment(selectedItem.comments_url, selectedItem).enqueue(responseBodyCallback);
                } else {
    ...
    private Callback<List<GithubRepo>> repoCallback = new Callback<List<GithubRepo>>() {
        @Override
        public void onResponse(Call<List<GithubRepo>> call, Response<List<GithubRepo>> response) {
            if (response.isSuccessful()) {
                List<GithubRepo> value = response.body();
                if (!value.isEmpty()) {
    ...
    private Callback<List<GithubIssue>> issuesCallBack =  new Callback<List<GithubIssue>>() {
        @Override
        public void onResponse(Call<List<GithubIssue>> call, Response<List<GithubIssue>> response) {
            if (response.isSuccessful()) {
                List<GithubIssue> value =  response.body();
                if (!value.isEmpty()) {
